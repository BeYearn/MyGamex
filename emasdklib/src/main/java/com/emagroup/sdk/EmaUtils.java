...
import com.emagroup.sdk.coolpad.EmaUtilsCoolPadImpl;
...
public class EmaUtils {
    ...
    private void realInit(JSONObject data) {
        if("000078".equals(ULocalUtils.getChannelId(activity))){ 
            EmaUtilsCoolPadImpl.getInstance(activity).realInit(mListener,data);
        }
    }
    ...
    public void realLogin(EmaSDKListener listener, String userid, String deviceKey) {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   
            EmaUtilsCoolPadImpl.getInstance(activity).realLogin(listener,userid,deviceKey);
        }

    }

    public void logout() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).logout();
        }
    }

    public void swichAccount() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).swichAccount();
        }
    }

    public void doPayPre(EmaSDKListener listener) {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).doPayPre(listener);
        }
    }

    public void realPay(EmaSDKListener listener, EmaPayInfo emaPayInfo) {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).realPay(listener,emaPayInfo);
        }
    }

    public void doShowToolbar() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).doShowToolbar();
        }
    }

    public void doHideToobar() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).doHideToobar();
        }
    }

    public void onResume() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).onResume();
        }
    }

    public void onPause() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).onPause();
        }
    }

    public void onStop() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).onStop();
        }
    }

    public void onDestroy() {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).onDestroy();
        }
    }

    public void onBackPressed(EmaBackPressedAction action) {
        if("000078".equals(ULocalUtils.getChannelId(activity))){   //酷派渠道
            EmaUtilsCoolPadImpl.getInstance(activity).onBackPressed(action);
        }
    }

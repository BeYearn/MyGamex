...
public class EmaUtilsImpl implements EmaUtilsInterface {
    ...
    public void submitGameRole(Map<String, String> data) {
        try {
            ...
            params.put(SDKParamKey.STRING_ROLE_ID, data.get(EmaConst.SUBMIT_ROLE_ID));
            params.put(SDKParamKey.STRING_ROLE_NAME, data.get(EmaConst.SUBMIT_ROLE_NAME));
            params.put(SDKParamKey.LONG_ROLE_LEVEL, Long.parseLong(data.get(EmaConst.SUBMIT_ROLE_LEVEL)));   // 坑爹！！的long型
            params.put(SDKParamKey.LONG_ROLE_CTIME, Long.parseLong(data.get(EmaConst.SUBMIT_ROLE_CT)));
            params.put(SDKParamKey.STRING_ZONE_ID, data.get(EmaConst.SUBMIT_ZONE_ID));
            params.put(SDKParamKey.STRING_ZONE_NAME, data.get(EmaConst.SUBMIT_ZONE_NAME));
            try {
                UCGameSdk.defaultSdk().submitRoleData(mActivity, params);
                Log.e("UCsubmitRole", params.toString());
            } catch (Exception e) {
                //传入参数错误异常处理
                Log.e("UCsubmitRole_error", e.toString());
            }
